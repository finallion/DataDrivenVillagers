...
500 kB is not sent and the log shows a warning. Put images of that size into a resource pack.
...
In 1.20.1, only two villager types have a hat of their own, `minecraft:desert` and `minecraft:snow`,
...
available. Forge is not affected, because registration there is split across the matching
`RegisterEvent`s.
